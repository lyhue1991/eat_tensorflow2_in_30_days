...
第二种方法更加接近TensorFlow原生方式，相对也更加简单一些。
...
def split_line(line):
    ...
    label = tf.reshape(tf.cast(tf.strings.to_number(arr[0]),tf.int32),(-1,))
    text = tf.reshape(arr[1],(-1,))
    return (text,label)
...
```
Valid Loss:0.0191127658,Valid Accuracy:0.99425
```
...
[[8.1336805e-07]
 ...
 [4.4090154e-09]
 [9.2865968e-01]
 [1.8057785e-06]], shape=(20, 1), dtype=float32)
```
