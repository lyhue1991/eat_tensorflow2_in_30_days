...
```python
# 演示自定义模型范例，实际上应该优先使用Sequential或者FunctionalAPI
...
训练模型通常有3种方法，内置fit方法，内置tran_on_batch方法，以及自定义训练循环。此处我们通过自定义训练循环训练模型。
...
自定义训练循环时的模型没有经过编译，无法直接使用model.evaluate(ds_valid)方法
